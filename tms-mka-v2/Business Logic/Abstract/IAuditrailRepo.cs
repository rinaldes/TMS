...
namespace tms_mka_v2.Business_Logic.Abstract
{
    ...
    {
        OrderHistory getHistoryUnderSpecificUser(Context.SalesOrder so, DateTime ProcessedAt, int userId);
        void save(Auditrail dbitem);
        void saveBTHistory(Context.SalesOrder so, String listSo = null);
        List<Auditrail> FindAll(int? skip = null, int? take = null, List<SortingInfo> sortings = null, FilterInfo filters = null);
        String FindAllNativeQuery(int? skip = null, int? take = null, List<SortingInfo> sortings = null, FilterInfo filters = null);
        OrderHistory getKasirHistoryUnder1Minutes(Context.SalesOrder so, DateTime ProcessedAt);
        int Count(FilterInfo filters = null);
        Auditrail FindByPK(int id);
        void delete(Auditrail dbitem);
        void saveOrderHistory(Context.SalesOrder so);
        OrderHistory getAUJKontrakHistoryUnder1Minutes(Context.SalesOrder so, DateTime ProcessedAt, int userId);
        void saveInapHistory(Context.SolarInap si, int userId);
        void savePlanningHistory(Context.SalesOrder so, string listSo = null);
        void saveKonfirmasiHistory(Context.SalesOrder so, string listSo = null);
        void saveAUJHistory(Context.SalesOrder so, Context.HistoryJalanTruck hjt, Context.AdminUangJalan auj = null, String listSo=null);
        void saveKasirTfHistory(Context.SalesOrder so, string strQuery, String listSo = null);
        void saveMajuHistory(Context.SalesOrder so, string strQuery);
        void saveKasirKasHistory(Context.SalesOrder so, string strQuery, String listSo = null);
        void saveSettRegHistory(Context.SalesOrder so, String listSo = null);
